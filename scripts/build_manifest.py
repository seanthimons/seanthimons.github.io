...
Re-running this script always produces an up-to-date manifest — no manual
edits needed when new weeks are released.
"""
...
# Start date: first week we want to cover
...
# Hardcoded name cache — avoids meta.yaml fetch for known weeks.
# New weeks discovered via API will be fetched from meta.yaml automatically.
KNOWN_NAMES = {
...
def gh_api_json(endpoint: str) -> list | None:
    """Call the GitHub API via gh CLI and return parsed JSON, or None on error."""
    try:
        result = subprocess.run(
            ["gh", "api", f"repos/{REPO}/contents/{endpoint}"],
            capture_output=True, text=True, timeout=30,
...
def discover_weeks_for_year(year: int) -> list[str]:
    """List all date directories under data/{year}/ from the GitHub repo.

    Returns a sorted list of date strings like ['2025-02-25', '2025-03-04', ...].
    """
    entries = gh_api_json(f"data/{year}")
    if entries is None:
        return []
    ...
    for entry in entries:
        name = entry.get("name", "")
        if entry.get("type") == "dir" and date_pattern.match(name):
            dates.append(name)
...
def fetch_week_metadata(year: int, date_str: str) -> dict | None:
    """Fetch meta.yaml for a given week and parse it."""
    raw = gh_api(f"data/{year}/{date_str}/meta.yaml")
    if raw is None:
        return None
...
def iso_week_number(date_str: str) -> int:
    """Get ISO week number from a date string like '2025-03-04'."""
...
def scan_existing_posts_for_2024_datasets(posts_dir: Path, datasets_2024: list, names_2024: dict) -> set:
    ...
    date_to_idx = {}
    for idx, (week, d) in enumerate(datasets_2024):
        date_to_idx[d] = idx

    # Also build name -> index lookup for matching by dataset name
    name_to_idx = {}
    for idx, (week, d) in enumerate(datasets_2024):
        name = names_2024.get(d, "")
        if name:
...
def build_manifest():
    ...
    for date_str, year in all_dates:
        week_num = iso_week_number(date_str)
        name = KNOWN_NAMES.get(date_str, "")
        byod = is_byod(name) if name else False
        ...
        entry = {
            ...
            "dataset_name": name,
            "dataset_slug": slugify(name) if name else "",
            "data_source_url": f"https://github.com/{REPO}/tree/main/data/{year}/{date_str}",
            "is_byod": byod,
            "substituted_from": None,
        }
        manifest.append(entry)

    # Fetch meta.yaml for weeks missing from the name cache
    uncached = [e for e in manifest if not e["dataset_name"] and not e["is_byod"]]
    if uncached:
        print(f"\nFetching meta.yaml for {len(uncached)} newly discovered week(s)...")
    for entry in uncached:
        meta = fetch_week_metadata(entry["year"], entry["week_date"])
        if meta and "title" in meta and meta["title"]:
            entry["dataset_name"] = meta["title"]
            entry["dataset_slug"] = slugify(meta["title"])
            entry["is_byod"] = is_byod(meta["title"])
            if "data_source_url" in meta:
                entry["data_source_url"] = meta["data_source_url"]
            print(f"  [OK] {entry['week_date']}: {entry['dataset_name']}")
        else:
            print(f"  [--] {entry['week_date']}: meta.yaml not found or has no title")

    # Also update data_source_url from meta.yaml for cached weeks
    # (only if they still have the generic GitHub URL)
    cached_with_generic_url = [
        e for e in manifest
        if e["dataset_name"]
        and not e["is_byod"]
        and "github.com" in e.get("data_source_url", "")
    ]
    if cached_with_generic_url:
        print(f"\nFetching detailed metadata for {len(cached_with_generic_url)} cached week(s)...")
        for entry in cached_with_generic_url:
            meta = fetch_week_metadata(entry["year"], entry["week_date"])
            if meta:
                ...
                print(f"  [--] {entry['week_date']}: using cached name")

    # Build names_2024 lookup from KNOWN_NAMES for the scan
    names_2024 = {d: KNOWN_NAMES[d] for _, d in DATASETS_2024 if d in KNOWN_NAMES}

    # Rescan existing posts to find which 2024 datasets are already used
    ...
    if already_used:
        ...
        for idx in sorted(already_used):
            week, d = DATASETS_2024[idx]
            print(f"  - {names_2024.get(d, d)} (2024 week {week})")
    ...
    # Flag any entries still without a name (shouldn't happen, but be safe)
    unnamed = [e for e in manifest if not e["dataset_name"]]
    if unnamed:
        print(f"\nWARNING: {len(unnamed)} week(s) have no dataset name:")
        for e in unnamed:
            print(f"  - {e['week_date']}")
