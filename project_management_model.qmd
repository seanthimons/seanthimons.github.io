---
title: "Mental Model"
subtitle: "How I think about projects: principles, tradeoffs, and risks"
format:
  html:
    page-layout: full
    toc: false
...
mermaidDef = `
flowchart TB
    subgraph motivation["🎯 MOTIVATION (4 Types)"]
        M1[Captivate]
        M2[Create]
        M3[Compete]
        M4[Complete]
    ...
    subgraph problem_forming["🔍 PROBLEM FORMING (Not an Oracle!)"]
        PF1["Why? (5 Whys - recursive)"]
        PF2[Current State]
        PF3[Gaps]
        PF4[Desired Future State]
        PF5["Success Criteria:<br/>• Describe end state<br/>• Spec + conditions"]
    ...
    subgraph iron_triangle["⚖️ PROJECT CONSTRAINTS"]
        SCOPE["SCOPE<br/>(For who?)"]
        TIME["TIME<br/>• Tempo/Cadence<br/>• Fast & Often"]
        RESOURCES[RESOURCES]
        ...
        PARKINSON["⚠️ Parkinson's Law:<br/>Work expands to fill time"]
        TIME --- PARKINSON
        
        OPP_COST["💰 Opportunity Cost<br/>(What are you NOT doing?)"]
        OPP_COST -.->|"Applies to all"| SCOPE
        OPP_COST -.->|"Applies to all"| TIME
        OPP_COST -.->|"Applies to all"| RESOURCES
    end

    subgraph roles["👤 ROLES & KNOWLEDGE"]
        FACTOTUM["Factotum<br/>(Jack of all trades)"]
        BUS_FACTOR["🚌 Bus Factor<br/>(Knowledge distribution risk)"]
    ...
    subgraph design_principles["🏗️ DESIGN PRINCIPLES"]
        DRY["DRY: Don't Repeat Yourself<br/>Single representation of knowledge"]
        DRY_SUB["• Don't duplicate code<br/>• Don't duplicate intent<br/>• Don't duplicate structure for knowledge"]
        YAGNI["YAGNI:<br/>You Aren't Gonna Need It"]
        DESIGN_WEAR["Design vs Wear/Tear<br/>(Intentional vs Emergent)"]
        TECH_DEBT["💳 Technical Debt<br/>(Accumulated wear)"]
    ...
    subgraph scope_tools["🎯 SCOPING STRATEGIES"]
    ...
    subgraph output_quality["📦 OUTPUT QUALITY"]
        ...
        ROI["ROI → Usefulness = f(tech)"]
    end

    subgraph perf_tuning["⚡ PERFORMANCE (Slow? Wrong?)"]
    ...
    subgraph tradeoffs["📊 TRADEOFFS"]
        TRADEOFF_3D["Accuracy ↔ Reliability ↔ Certainty<br/>(Can't maximize all three)"]
    end

    subgraph project_risks["⚠️ PROJECT RISKS"]
        YAK_SHAVING["🐃 Yak Shaving<br/>(Recursive prerequisite tasks:<br/>To do X, first must do Y, Z...)"]
        SANDBAGGING["Sandbagging<br/>(Inflated estimates)"]
        SCOPE_CREEP["Scope Creep"]
        SALT["🧂 Salt Principle:<br/>If something spoils, add salt to fix it.<br/>But woe to the day the salt itself has spoiled.<br/>(The fix becomes the problem)"]
        GREENER["🌱 Greener Grass Trap<br/>(The grass is greener<br/>where you water it)"]

        YAK_SHAVING -->|"Causes"| SCOPE_CREEP
        GREENER -->|"Causes"| SCOPE_CREEP
    end

    subgraph communication["💬 COMMUNICATION"]
        COMPLAIN["Complain + Propose"]
        COMPLAIN_SUB["• Is it a big deal?<br/>• Bundle proposal with complaint<br/>• Reverse roles, receive as other person"]
    ...
    class YAK_SHAVING,SCOPE_CREEP,TECH_DEBT,SALT,GREENER risk
...
// ===== CONFIGURATION: Control initial diagram position =====
// Modify these values to change where the diagram starts rendering:
// - initialScale: zoom level (0.1 to 3.0, where 1.0 is 100%)
// - initialPanX: horizontal offset in pixels (positive = right, negative = left; 0 = no horizontal offset)
// - initialPanY: vertical offset in pixels (positive = down, negative = up; 0 = no vertical offset)
...
viewof controls = {
  const container = html`<div style="display: flex; justify-content: center; gap: 10px; margin-bottom: 15px;">
    <button id="zoomIn" style="background: #0D5C63; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: 500;">➕ Zoom In</button>
    <button id="zoomOut" style="background: #0D5C63; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: 500;">➖ Zoom Out</button>
    <button id="reset" style="background: #0D5C63; color: white; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: 500;">↺ Reset</button>
    <button id="fitView" style="background: #78CDD7; color: #1A2E35; border: none; padding: 8px 16px; border-radius: 6px; cursor: pointer; font-weight: 500;">⊡ Fit to View</button>
...
<p style="color: #666; font-size: 0.8rem; text-align: center; margin: 10px 0;">
  🖱️ Scroll to zoom • Click and drag to pan • Use buttons for precise control
</p>
