...
wq <- water_quality %>%
  ...
  mutate(
    ...
    rain_category = factor(rain_category, levels = c(
      "No rain", "Light (0-5mm)", "Moderate (5-20mm)",
      "Heavy (20-50mm)", "Extreme (50mm+)"
    ))
  )

cat("Joined dataset:", nrow(wq), "rows\n")
cat("Date range:", as.character(range(wq$date)), "\n")
cat("Rainfall match rate:", scales::percent(mean(!is.na(wq$precipitation_mm))), "\n")
...
site_stats <- wq %>%
  ...
  mutate(
    ...
    quality_grade = factor(quality_grade, levels = c("Good", "Fair", "Poor", "Very Poor"))
  )

# Top 10 worst sites by exceedance rate
site_stats %>%
  arrange(desc(pct_exceedance)) %>%
  select(region, swim_site, n_samples, median_entero, p90_entero, pct_exceedance, quality_grade) %>%
  head(10)
...
cat("  1992:", round(yearly_overall$geo_mean[yearly_overall$year == 1992], 1), "\n")
cat("  2024:", round(yearly_overall$geo_mean[yearly_overall$year == max(yearly_overall$year)], 1), "\n")
...
region_colors <- setNames(
  ...
  c("Western Sydney", "Sydney Harbour", "Sydney City", "Northern Sydney", "Southern Sydney")
)
...
p_trend <- ggplot(yearly_trends, aes(x = year, y = geo_mean, color = region)) +
  ...
  geom_hline(yintercept = 40, linetype = "dashed", color = "grey40", linewidth = 0.5) +
  annotate("text", x = 1993, y = 45, label = "40 CFU threshold",
...
# Shared scales and theme for both panels
map_base <- function(data, xlim, ylim, title) {
  ggplot() +
    geom_sf(data = aus, fill = "grey90", color = "grey50", linewidth = 0.3) +
    geom_sf(data = data, aes(color = quality_grade, size = p90_entero), alpha = 0.75) +
    ...
    labs(title = title, color = "Quality Grade") +
...
p_coast <- map_base(
  coastal_sf,
  xlim = c(as.numeric(coastal_bbox["xmin"]) - 0.02, as.numeric(coastal_bbox["xmax"]) + 0.02),
  ylim = c(as.numeric(coastal_bbox["ymin"]) - 0.02, as.numeric(coastal_bbox["ymax"]) + 0.02),
  title = "Coastal & Harbour Sites"
)

# Western Sydney panel — river swim sites
western_bbox <- st_bbox(western_sf)
p_west <- map_base(
  western_sf,
  xlim = c(as.numeric(western_bbox["xmin"]) - 0.03, as.numeric(western_bbox["xmax"]) + 0.03),
  ylim = c(as.numeric(western_bbox["ymin"]) - 0.03, as.numeric(western_bbox["ymax"]) + 0.03),
  title = "Western Sydney River Sites"
)

# Combine with shared legend
p_map <- p_coast + p_west +
  plot_layout(guides = "collect") +
  plot_annotation(
    title = "Sydney Swim Site Water Quality Grades",
    subtitle = "Color = overall quality grade | Size = 90th percentile enterococci",
...
worst_trends <- wq %>%
  filter(swim_site %in% worst_sites, !is.na(enterococci_cfu_100ml), enterococci_cfu_100ml > 0) %>%
...
p_spark <- ggplot(worst_trends, aes(x = year, y = geo_mean)) +
  ...
  geom_smooth(se = FALSE, method = "loess", span = 0.4,
              color = as.character(hiroshige[1]), linewidth = 0.9) +
  geom_hline(yintercept = 40, linetype = "dotted", color = "grey50", linewidth = 0.3) +
  facet_wrap(~swim_site, ncol = 2, scales = "free_y") +
...
# Combine map and sparklines with patchwork
hero <- p_map + p_spark +
...
ggsave(
  here::her
	e("posts", "2025-05-20", "hero.png"),
  plot = hero,
  width = 12, height = 10, units = "in", dpi = 320,
  bg = "white"
)
...
write.table(
  ...
  here::here("posts", "palette-log.csv"),
